...
We identified 7 PII fields stored without protection, including 497 SSNs in plain text (99.0% of all records). The dataset has no consent tracking mechanism, no data retention policy, and no audit trail for automated credit decisions.
...
| Art. 9: Special Categories | Gender collected without documented exception; spending data can reveal health/addiction | High |
| Art. 17: Right to Erasure | 497 SSNs in plain text - erasure requests unfulfillable | Critical |
| Art. 22: Automated Decisions | All decisions automated - rejection reason is algorithm_risk_score with no human review | Critical |
...
SHA-256 hashing with a salt was applied to SSN, name, and email fields across all 502 records. This prevents casual identification and supports duplicate detection, but pseudonymized data remains personal data under GDPR - it is not true anonymization.
...
6. **Quarterly bias audits**: Compute disparate impact ratios for gender and age regularly; DI below 0.8 triggers automatic review (AI Act Art. 10)
